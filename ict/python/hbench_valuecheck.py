...
if __name__ == '__main__':
    ...
    for i in range(repeats):
        ...
        circuit_single = build_circuit(args, 1, pairs)
    ...
    for idx in range(numElements):
        ...
        if val_ref.real == 0.0 or val_ref.imag == 0:
            if abs(val.real - val_ref.real) > 1.0e-15 or abs(val.imag - val_ref.imag) > 1.0e-15:
                print( "#rank: ", rank, " , idx: ", offset+idx, "ref val: ", val_ref, " , val: ", val)
                checkFlag = False
                break

        else:
            if (abs(val.real - val_ref.real)/abs(val_ref.real)) > 1.0e-15 or (abs(val.imag - val_ref.imag)/abs(val_ref.imag)) > 1.0e-15:
                print( "#rank: ", rank, " , idx: ", offset+idx, "ref val: ", val_ref, " , val: ", val)
                checkFlag = False
                break
