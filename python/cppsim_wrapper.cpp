...
PYBIND11_MODULE(qulacs, m) {
        ...
        .def("get_matrix", [](const DensityMatrix& state) {
            ...
            for (ITYPE y = 0; y < state.dim; ++y) {
                for (ITYPE x = 0; x < state.dim; ++x) {
                    mat(y, x) = (std::complex<double>)ptr[y*state.dim + x];
                }
            }
            return mat;
        }, "Get density matrix")
    ...
    // for mpiQulacs
    mgate.def("FusedSWAP", [](UINT target_index1, UINT target_index2, UINT num_qubits) {
        auto ptr = gate::FusedSWAP(target_index1, target_index2, num_qubits);
        if (ptr == NULL) throw std::invalid_argument("Invalid argument passed to FusedSWAP.");
        return ptr;
    }, pybind11::return_value_policy::take_ownership, "Create FusedSWAP gate", py::arg("target1"), py::arg("target2"), py::arg("numqubits")); 
    //
    ...
    py::class_<QuantumCircuit>(m, "QuantumCircuit")
        ...
        // for mpiQulacs
        .def("add_FusedSWAP_gate", &QuantumCircuit::add_FusedSWAP_gate, "Add FusedSWAP gate", py::arg("target1"), py::arg("target2"), py::arg("numqubits"))
        //
