...
void _ApplyDoubleDenseGate(UINT n, UINT target1, UINT target2);

TEST(GateTest_multicpu, ApplyDoubleDenseGate_8qubit_all) {
    for (UINT n = 2; n <= 8; ++n) {
        for (UINT c = 0; c < n; ++c) {
            for (UINT t = 0; t < n; ++t) {
                if (c == t) continue;
                _ApplyDoubleDenseGate(n, c, t);
            }
        }
    }
}
...
void _ApplyDoubleDenseGate(UINT n, UINT target1, UINT target2) {
...

    {
        ...
        // update state
        auto gate1 = gate::RZ(target1, -0.5);
        auto gate2 = gate::CNOT(target1, target2);
        gate1->update_quantum_state(&state);
        gate2->update_quantum_state(&state);

        auto merged_gate =
            gate::merge(gate::RZ(target1, -0.5), gate::CNOT(target1, target2));
