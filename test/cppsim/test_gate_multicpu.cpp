...
void _ApplyTwoQubitGate(UINT n, UINT control, UINT target,
    ...
    Eigen::VectorXcd test_state1 = Eigen::VectorXcd::Zero(dim);
    {
        ...
        // std::cout << "# rank,tgt,cnt: " << m->get_rank() << "," << target <<
        // "," << control
        //	<< ":0 " << state.data_cpp()[0] << ", " << test_state1[0+offs]
        //	<< ":1 " << state.data_cpp()[1] << ", " << test_state1[1+offs]
        //	<< std::endl;
        //	<< ": " << state.data_cpp()[32%inner_dim] << ", " <<
        //test_state1[(32+offs)%dim] << ", " <<
        //state_ref.data_cpp()[(32+offs)%dim]
        //	<< ": " << state.data_cpp()[64%inner_dim] << ", " <<
        //test_state1[(64+offs)%dim] << ", " <<
        //state_ref.data_cpp()[(64+offs)%dim]
        for (ITYPE i = 0; i < inner_dim; ++i)
            ASSERT_NEAR(
...
ASSERT_NEAR(abs(state.data_cpp()[i] - test_state.data_cpp()[i]), 0, eps);
        ...
        for (UINT gate_index = 0; gate_index < gate_count; ++gate_index) {
            ...
            //std::cout << "***************************************************"
<< std::endl;
            //std::cout << " ***** Pauli = " << new_pauli_id << " at " << target
<< std::endl;
...
get_eigen_matrix_single_Pauli(new_pauli_id), n); total_matrix = new_gate_matrix
* total_matrix; test_state_eigen = new_gate_matrix * test_state_eigen;
            ...
            if (check_mat.rows() == total_matrix.rows()) {
                for (ITYPE x = 0; x < dim; ++x) {
                    for (ITYPE y = 0; y < dim; ++y) {
                        ASSERT_NEAR(abs(total_matrix(x, y) - check_mat(x, y)),
0, eps) << (QuantumGateMatrix*)merged_gate << std::endl << "current eigen matrix
...
(QuantumGateMatrix*)merged_gate << std::endl << "current eigen matrix : \n" <<
total_matrix << std::endl;
            //std::cout << "***************************************************"
<< std::endl;
...
<< std::endl;
            ...
            auto new_gate_matrix =
get_expanded_eigen_matrix_with_identity(target,
cos(angle/2)*ComplexMatrix::Identity(2,2) + 1.i * sin(angle/2)*
get_eigen_matrix_single_Pauli(new_pauli_id), n); total_matrix = new_gate_matrix
* total_matrix; test_state_eigen = new_gate_matrix * test_state_eigen;
...
ASSERT_NEAR(abs(state.data_cpp()[i] - test_state.data_cpp()[i]), 0, eps);
        ...
        for (UINT gate_index = 0; gate_index < gate_count; ++gate_index) {
            ...
            ComplexMatrix mat = di * get_eigen_matrix_single_Pauli(0)
+ 1.i*(dx*get_eigen_matrix_single_Pauli(1) + dy *
get_eigen_matrix_single_Pauli(2) + dz * get_eigen_matrix_single_Pauli(3));
            ...
            // update eigen state with matrix mul
            auto new_gate_matrix =
get_expanded_eigen_matrix_with_identity(target, mat, n); total_matrix =
new_gate_matrix * total_matrix; test_state_eigen = new_gate_matrix *
test_state_eigen;
