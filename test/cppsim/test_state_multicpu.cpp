...
TEST(StateTest_multicpu, GenerateAndRelease) {
    ...
    for (UINT i = 0; i < state.dim; ++i) {
        if (i == 0 and state.device_number == 0)
    ...
    for (UINT repeat = 0; repeat < 10; ++repeat) {
        ...
        // std::cout << "#GenAndRelease; " << state.device_number << ", " <<
        // basis << ", " << state.outer_qc << std::endl;
        state.set_computational_basis(basis);
        for (UINT i = 0; i < state.dim; ++i) {
            if (i == basis and i >> state.inner_qc == state.device_number)
                ASSERT_NEAR(abs(state.data_cpp()[i] - 1.), 0, eps);
            else
                ASSERT_NEAR(abs(state.data_cpp()[i]), 0, eps);
        }
...
TEST(StateTest_multicpu, LoadState) {
    ...
    for (UINT repeat = 0; repeat < 10; ++repeat) {
        ...
        for (ITYPE i = 0; i < inner_dim; ++i)
            ASSERT_NEAR(
                abs(state_multi_ref.data_cpp()[i] - state.data_cpp()[i + offs]),
                0, eps)
                << "rank: " << m->get_rank() << " idx: " << i + offs
                << " stete: " << state.data_cpp()[i + offs]
