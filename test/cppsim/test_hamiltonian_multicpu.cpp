...
TEST(ObservableTest_multicpu, CheckExpectationValue) {
    ...
    QuantumState ref_state(n, 0);
    ref_state.set_computational_basis(0);
...
TEST(ObservableTest_multicpu, CheckParsedObservableFromOpenFermionText){
...
TEST(ObservableTest_multicpu, CheckSplitObservable){
    auto func = [](const std::string path, const QuantumStateBase* state) -> CPPCTYPE {
            std::ifstream ifs;
            CPPCTYPE coef;
            ifs.open(path);
