...
public:
    ...
    virtual void load(const QuantumStateBase* _state) {
        if (_state->qubit_count != this->qubit_count) {
            std::cerr << "Error: DensityMatrixCpu::load(const "
    ...
    virtual void load(const std::vector<CPPCTYPE>& _state) {
        if ((ITYPE)_state.size() != _dim && _state.size() != _dim * _dim) {
            std::cerr << "Error: DensityMatrixCpu::load(vector<Complex>&): "
    ...
    virtual void load(const ComplexMatrix& _state) {
        if ((ITYPE)_state.cols() != _dim &&
            (ITYPE)_state.rows() != _dim * _dim) {
            std::cerr << "Error: DensityMatrixCpu::load(ComplexMatrix&): "
                         "invalid length of state"
                      << std::endl;
            return;
        }
    ...
    virtual void add_state(const QuantumStateBase* state) override {
        if (state->is_state_vector()) {
            std::cerr << "add state between density matrix and state vector is "
                         "not implemented"
                      << std::endl;
            return;
        }
