    ...
    while (!ss.eof()) {
        ...
        if (index == UINT_MAX - 1) {
            if (pauli_str.empty() == false) {
                std::cerr
                    << "Warning: PauliOperator::PauliOperator(std::string, "
                       "CPPCTYPE) : detected pauli_str without indexs. Maybe "
                       "mistyped?"
                    << std::endl;
...
CPPCTYPE PauliOperator::get_expectation_value(
    ...
    if (state->qubit_count < this->get_qubit_count()) {
        std::cerr
            << "Error: PauliOperator::get_expectation_value(QuantumStateBase*) "
               ": The number of qubit in PauliOperator is greater than "
               "QuantumState. PauliOperator: "
            << this->get_qubit_count()
            << " QuantumState: " << state->qubit_count << std::endl;
