...
QuantumGateBase* BSWAP(UINT qubit_index1, UINT qubit_index2, UINT num_qubits) {
    if (((qubit_index1 <= qubit_index2) && (qubit_index1 + num_qubits > qubit_index2)) ||
        ((qubit_index2 <= qubit_index1) && (qubit_index2 + num_qubits > qubit_index1)))
    {
...
QuantumGateBase* SparseMatrix(
    ...
    if (!check_is_unique_index_list(target_list)) {
        std::cerr << "Error: gate::SparseMatrix(std::vector<UINT> target_list, "
                     "SparseComplexMatrix matrix): target list contains "
                     "duplicated values."
                  << std::endl;
        return NULL;
    }
...
QuantumGateBase* DiagonalMatrix(
    ...
    if (!check_is_unique_index_list(target_list)) {
        std::cerr << "Error: gate::DiagonalMatrix(std::vector<UINT> "
                     "target_list, ComplexVector diagonal_element): target "
                     "list contains duplicated values."
                  << std::endl;
        return NULL;
    }
...
QuantumGateMatrix* RandomUnitary(std::vector<UINT> target_list) {
    if (!check_is_unique_index_list(target_list)) {
        std::cerr << "Error: gate::RandomUnitary(std::vector<UINT> "
                     "target_list): target list contains duplicated values."
                  << std::endl;
        return NULL;
    }
    Random random;
...
QuantumGateBase* TwoQubitDepolarizingNoise(
    ...
    if (target_index1 == target_index2) {
        std::cerr << "Error: gate::TwoQubitDepolarizingNoise(UINT, UINT, "
                     "double): target list contains duplicated values."
                  << std::endl;
        return NULL;
    }
