...
#ifdef _USE_SIMD
    CZ_gate_simd(control_qubit_index, target_qubit_index, state, dim);
#else
    CZ_gate_unroll(control_qubit_index, target_qubit_index, state, dim);
#endif
...
void CZ_gate_unroll(UINT control_qubit_index, UINT target_qubit_index,
...
#ifdef _USE_SIMD
void CZ_gate_simd(UINT control_qubit_index, UINT target_qubit_index,
    CTYPE *state, ITYPE dim) {
    const ITYPE loop_dim = dim / 4;
