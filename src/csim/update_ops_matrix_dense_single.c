    ...
    if(dim >= vec_len){
        ...
        if (flag){
            ...
            for (ITYPE state_index = 0; state_index < dim; state_index += vec_len) {
                ...
                input2 = svld1_f64(pg, (double *)&t[state_index+(vec_len>>1)]);
                input3 = svld1_f64(pg, (double *)&state[state_index+(vec_len>>1)]);
            ...
            for (ITYPE state_index = 0; state_index < dim; state_index += vec_len) {
                ...
                input2 = svld1_f64(pg, (double *)&state[state_index+(vec_len>>1)]);
                input3 = svld1_f64(pg, (double *)&t[state_index+(vec_len>>1)]);
...
#pragma omp parallel for private(input0, input1, input2, input3,  \
                                 output0, output1, result01_real, result01_imag, \
                                 cval02_real, cval02_imag, cval13_real, cval13_imag) \
